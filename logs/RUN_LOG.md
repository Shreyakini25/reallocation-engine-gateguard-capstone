...
## 2026-07-04 — case-tpm-ai-infra-sponsor RUNNABLE-SAMPLE run (Mode Build assignment)

- **Recipe:** case-tpm-ai-infra-sponsor v0.1.0 (new — first non-DRAFT recipe in the repo)
- **Mode:** sample; no writes to source data; scorer's default output paths in tmp/
- **Inputs:**
    - tmp/tpm-sample-roles.json — 5 real companies from data/80-days-to-stay/data/SEC_DOL_H1b_data_mapped.csv, filtered on top_job_titles_sponsored containing "Product Manager" or "Program Manager" (ACCELA, ACCELBYTE, ACORNS GROW, AIERA, AIRBYTE)
    - tmp/broken-roles.json — soft break attempt (schema-invalid: missing sponsorship field + string-instead-of-number)
    - tmp/broken-roles-hard.json — hard break attempt (parser-invalid JSON: unterminated string + trailing comma)
    - 1 real Bloomberg TPM posting URL for the liveness gate
- **Outputs:**
    - tmp/role-scores.json + tmp/role-scores.md (scorer output, generated by npm run score)
    - recipes/case-tpm-ai-infra-sponsor.md (mode file, RUNNABLE-SAMPLE)
    - assignments/submissions/saloni/domain-justification.md
    - assignments/submissions/saloni/worked-run.md
- **Result:**
    - Real-data scoring: 5 roles → Apply 4 · Consider 0 · Skip 1 (skip 20%); ACORNS Skipped cleanly on liveness=0, replicating Ch.11 ghost-posting behavior
    - Soft break attempt (schema-invalid): 2 roles → Skip 2 (100%); scorer refused to guess. Hard break attempt (parser-invalid JSON): SyntaxError at JSON.parse (line 2 col 37) — clean crash. Together the two attempts characterize the scorer's failure surface (soft-fail on schema, hard-fail on parse; neither guesses)
    - Liveness gate: Bloomberg TPM URL returned ✅ active
    - CSV-level filter evidence: grep "product manager" → 107 rows across the master CSV (~6.9% of the 1,557 sponsorship-populated rows)
- **Flags:** 20% skip rate below the ~50% healthy target; sample is deliberately biased toward companies with sponsorship data (all 5 have Approval_Rate 100.0), so a full-CSV run would push skip rate above 90% at the sponsorship-populated gate alone
- **Gates cleared:** data-present, sponsorship-populated (fixture), title-match (grep-verified), timeline (fixture profile), liveness (real URL executed), sparsity-reported (funnel documented in worked-run doc)
- **Open:**
    - `[TODO DEV]` scripts/tpm/filter-tpm-candidates.py — CSV-to-roles.json still manual
    - `[TODO DEV]` scripts/tpm/tpm-title-taxonomy.md — no collision screening in this run (Failure Mode 1 unscreened)
    - `[TODO DATA SOURCE]` SOC-code enrichment on H-1B data
    - Full-CSV funnel not yet run; 20% skip rate is unrepresentative of the mode's expected production behavior
    - Human adequacy attestation (P4 second half) not yet obtained — required to promote past RUNNABLE-SAMPLE to RUNNABLE-LIVE
