...
## 2026-07-06 -- Mode build: `case-mscs-opt-research-like-ai-software` (sample run)

- **Recipe:** `case-mscs-opt-research-like-ai-software` (F-1 MSCS/MSIS student, pre-OPT; research-like AI-era software roles), **sample mode**, run id `case-mscs-opt-research-like-ai-software-2026-07-06-001`. Renamed from the untracked draft `recipes/research-like job searching.md` (DRAFT, 12 open TODOs, all-proposed `.py` scripts) and re-grounded on a script that runs today. Status DRAFT → **RUNNABLE-SAMPLE**.
- **Command:** `npm run score -- data/examples/research-like-roles.json --profile data/examples/research-like-profile.json --out-dir reports/generated --md reports/generated/case-mscs-opt-research-like-ai-software-2026-07-06.md` (stored script `scripts/score/role-scorer.mjs`; no ad-hoc code). Second anchor: `npm run ats:scan -- --dry-run` (real live Greenhouse data, no writes).
- **Inputs:** anonymized fixtures `data/examples/research-like-roles.json` (8 roles) + `research-like-profile.json` (needs_sponsorship=true). No personal data.
- **Gates:** 1 Source ✓ · 2 Scope ✓ (sample) · 3 Data-shape ✓ · 4 Script-readiness ✓ · 5 Approval n/a · 6 Liveness ✓ (fired: ghost posting → Skip) · 7 Timeline ✓ (fired: early start → Skip) · 8 Report ✓. Human adequacy gate: **PENDING attestation** (student attestation drafted in submission worked-run).
- **Result:** 8 roles → Apply 2 · Consider 3 · Skip 3 (skip 38%). Two roles gated to composite 0.000 as designed. Control run on `ch11-roles.json` reproduces Ch.11. Output fully sourced (record / model-judgment / your-input).
- **Break tests:** missing file → exit 2; malformed JSON → exit 1 (halts, no garbage); override w/o reason → ignored+warned; citizen profile → sponsorship weight→0, skip 38%→50% (profile-conditional weighting verified).
- **Artifacts:** `recipes/case-mscs-opt-research-like-ai-software.md`, `logs/case-mscs-opt-research-like-ai-software-2026-07-06.json`, `reports/generated/case-mscs-opt-research-like-ai-software-2026-07-06.{md,json}`, `data/examples/research-like-{roles,profile}.json`, `assignments/submissions/zening-teng/{domain-justification,worked-run}.md`.
- **Fixed:** unbalanced json code fence in the original draft (conformance "unbalanced code fences (1)") — recipe now passes conformance.
- **Flags/open:** skip-rate 38% < 50% (curated fixture); `role_quality` weight 0 [VERIFY] drops the Ch.9 signal (gap #3); sponsorship/liveness values are illustrative fixtures, not live joins ([TODO: DATA SOURCE]); proposed `.py` enrichment scripts remain [TODO: DEV]. **Pre-existing env note (fixed):** `npm run verify` was failing 30 `.py` files + `metadata.yaml` because `python3` was not on PATH on this Windows box (only Anaconda `python`); conformance shells `python3 -m py_compile`. Fixed by a `python3.exe` shim in the on-PATH Anaconda folder → `node scripts/conformance.mjs` now exits 0 (`✓ all conform`). Unrelated to the mode (added files are `.json`/`.md`).
