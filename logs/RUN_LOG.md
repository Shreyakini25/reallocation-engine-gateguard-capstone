...
## 2026-07-06 — cv-integrity: recipe built, sample + live runs, break testing

- **Recipe:** case-cv-integrity-backend-h1b v0.1.0 (new; status RUNNABLE-SAMPLE)
- **Inputs:** data/examples/cv-integrity/sample-base-cv.json + sample-variant-backend.md / sample-variant-clean.md (sample); private/resume.json + private/variants/cv-backend.tex (live, gitignored)
- **Outputs:** logs/cv-integrity-2026-07-06.json, reports/generated/cv-integrity-2026-07-06.md (sample); live artifacts written back into private/reports/ only
- **Result:** sample drifted variant → 8 flags (incl. fabricated `etcd`), exit 1; sample clean variant → 0 flags, exit 0; live run → 3 flags, PROMOTE ClickHouse applied to base (round-trip: flag resolved). After the metadata-corpus fix, live re-run → 6 flags: the 4 skills my setup-exercise attestation removed (Kafka, Spark, Airflow, Jenkins) had been silently re-anchored by the attestation note itself — all 4 → DROP; ETL/GraphQL still open; render gate stays blocked (exit 1)
- **Verified:** exit codes captured without pipes; agent log parses (`python3 -m json.tool`); `rds`-in-"dashboards" substring trap does not anchor RDS; MTTD anchors via abbreviation-initials; `npm run resumes:pdf -- --all` rendered the 4 tracked sample CVs (after `npx playwright install chromium`)
- **Broke during testing, fixed:** all three stop-condition paths crashed (TDZ ReferenceError, exit 1) instead of the designed graceful exit 2 — found by the deliberate break attempt; fixed (hoisted counter), re-tested. Hostile re-reviews (context-free agent sessions; see ai-use-disclosure) found four more: a base with NO attestation block bypassed the stop (fixed — exit 2); a fabricated CRM chance-anchored via whole-corpus initials (fixed — initials scoped per field); metadata-corpus pollution let an attestation note re-anchor the claims it removed (fixed — metadata excluded from corpus; live impact above); and an unhandled crash exited 1, the gate-open code (fixed — global handlers, exit 2). Also: --mode now validated; live runs refuse artifact paths outside private/ (incl. the stop-log). Sample counts unchanged throughout (73/22/8 · 72/21/0)
- **Open issues:** ETL/GraphQL flags undecided (human); 3 typed TODOs open (apply-promote, labeled semantic-drift assist, dictionary provenance); .md bases bypass the attestation gate (JSON-only check — exercised, documented in the recipe)
